...
async function restoreFromZip(zipFile) {
    ...
    try {
        ...
        const audioFiles = [];
        let zipIconFile = null;  // FIX 1: Sadece playlist kapağı için, şarkılara atanmayacak
        ...
        zip.forEach((relativePath, file) => {
            ...
            if (isAudioFile(filename)) {
                ...
                zipIconFile = file;  // FIX 1: İkon sadece playlist kapağı olarak saklanacak
        ...
        // FIX 1: İkon dosyasını SADECE LocalStorage'a playlist kapağı olarak kaydet, şarkılara ATAMA
        if (zipIconFile) {
            const iconBlob = await zipIconFile.async('blob');
            const iconBase64 = await new Promise((resolve) => {
                const reader = new FileReader();
                reader.onload = () => resolve(reader.result);
                reader.readAsDataURL(iconBlob);
            });
            const resizedIcon = await resizeImage(iconBase64);
            try {
                localStorage.setItem('fk_playlist_cover_restored', resizedIcon);
        ...
        // Ses dosyalarını işle ve şarkıları oluştur (FIX 1: Kapak ataması yapılmaz)
        let restoredCount = 0;
        ...
        // FIX 1: İkon, şarkılara kapak olarak atanmaz. Sadece LocalStorage'da 'fk_playlist_cover_restored' anahtarında saklanır.
...
function renderBackupPanel() {
    el.backupPanel.innerHTML = `
        ...
        <div class="backup-section">
            ...
            <!-- FIX 4: Geri yükleme uyarı metni -->
            <p class="restore-warning-text"><i class="fa-solid fa-triangle-exclamation"></i> Mevcut bütün şarkılar ve fotoğraflar silinir</p>
...
function setupEventListeners() {
    ...
    // FIX 3: Playlist Kapak Değiştiğinde Sidebar'ı da Güncelle
        ...
        if (file) {
            ...
            reader.onload = async (ev) => {
                ...
                if (currentPlaylistIdForCover) {
                    try { 
                        localStorage.setItem('fk_playlist_cover_' + currentPlaylistIdForCover, resizedDataUrl); 
                        showToast("Kapak güncellendi");
                        renderPlaylistsSidebar(); // FIX 3: Sidebar küçük resmini hemen güncelle
                    }
...
// Masaüstü pencere kontrollerini gizle
